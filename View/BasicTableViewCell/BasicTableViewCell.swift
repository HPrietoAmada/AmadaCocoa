...
class BasicTableViewCell: UITableViewCell {
    ...
    var alert: Alert? {
        didSet {
            ...
            topLeftLabel.text = "Notification Type"

            if let message = alert.message {
                descriptionLabel.text = message
            }

            if let sendDate = alert.sendDate {
                let sendDateFormatted = sendDate.getFormattedDate(format: "MM/dd/yyyy hh:mm a")
                bottomLeftLabel.text = sendDateFormatted
            }

            topRightLabel.text = "HPrieto"
        }
    }
